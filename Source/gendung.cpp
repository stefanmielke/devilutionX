...
std::unique_ptr<byte[]> LoadLevelSOLData(size_t &tileCount)
{
	switch (leveltype) {
	case DTYPE_TOWN:
		if (gbIsHellfire)
			return LoadFileInMem("NLevels\\TownData\\Town.SOL", &tileCount);
		return LoadFileInMem("Levels\\TownData\\Town.SOL", &tileCount);
	case DTYPE_CATHEDRAL:
		if (currlevel < 17)
			return LoadFileInMem("Levels\\L1Data\\L1.SOL", &tileCount);
		return LoadFileInMem("NLevels\\L5Data\\L5.SOL", &tileCount);
	case DTYPE_CATACOMBS:
		return LoadFileInMem("Levels\\L2Data\\L2.SOL", &tileCount);
	case DTYPE_CAVES:
		if (currlevel < 17)
			return LoadFileInMem("Levels\\L3Data\\L3.SOL", &tileCount);
		return LoadFileInMem("NLevels\\L6Data\\L6.SOL", &tileCount);
	case DTYPE_HELL:
		return LoadFileInMem("Levels\\L4Data\\L4.SOL", &tileCount);
	default:
...
{
	size_t tileCount;
	std::unique_ptr<byte[]> pSBFile = LoadLevelSOLData(tileCount);

	for (unsigned i = 0; i < tileCount; i++) {
		byte bv = pSBFile[i];
		nSolidTable[i + 1] = (bv & byte { 0x01 }) != byte { 0 };
		nBlockTable[i + 1] = (bv & byte { 0x02 }) != byte { 0 };
		nMissileTable[i + 1] = (bv & byte { 0x04 }) != byte { 0 };
		nTransTable[i + 1] = (bv & byte { 0x08 }) != byte { 0 };
		nTrapTable[i + 1] = (bv & byte { 0x80 }) != byte { 0 };
		block_lvid[i + 1] = static_cast<uint8_t>((bv & byte { 0x70 }) >> 4);
	}
}
